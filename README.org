...
#+TITLE: Elisp Cheatsheet for Python Programmers
...
|--------+-------+----------------------------------------|
| ~is~     | ~eq~    |                                        |
| ~==~     | ~equal~ | Use Elisp ~equal~ for string comparison. |

Depending on the type of object to compare, you may have to resort to writing a custom comparison function.

Basic ordering of two values of the same type in Elisp can be done with the ~value<~ function.
...
| ~len(d)~                 | ~(map-length d)~               |                                                      |
| ~d[k]~                   | ~(map-elt d k)~                |                                                      |
| ~d[k] = v~, ~k ∉ d.keys()~ | ~(setq d (map-insert d k v))~  | ~k~ is not in ~d~. ~map-insert~ will not check uniqueness. |
| ~d[k] = v~, ~k ∈ d.keys()~ | ~(map-put! d k v)~             | ~k~ is in ~d~. Note that ~map-put!~ will mutate ~d~.         |
...
| ~d.items()~              | ~(map-pairs d)~                |                                                      |
| ~d.keys()~               | ~(map-keys d)~                 |                                                      |
| ~d.pop(k)~               |                              | Can be emulated using ~map-elt~ and ~map-delete~.        |
| ~d.popitem()~            |                              |                                                      |
| ~reversed(d)~            | ~(reverse (map-keys d))~       |                                                      |
...
| Python                          | Elisp              |
|---------------------------------+--------------------|
| ~d = dict([(k, v)])~, ~d = {k : v}~ | ~(setq d '(k v))~    |
| ~d[k]~                            | ~(plist-get d k)~    |
| ~d[k] = v~                        | ~(plist-put d k v)~  |
| ~k in d~                          | ~(plist-member d k)~ |
...
Python [[https://docs.python.org/3/library/functions.html#map][map]] function on an iterable.
...
Elisp equivalent using ~cl-mapcar~ from ~cl-lib~.
...
| Python          | Elisp                            | Notes                                                                                                                                                                                 |
|-----------------+----------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ~re.sub(p, r, s)~ | ~(replace-regexp-in-string p r s)~ |                                                                                                                                                                                       |
| ~re.match(p, s)~  | ~(string-match p s)~               | ~re.match~ and ~string-match~ have different APIs that accomplish the same thing. See [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Simple-Match-Data.html][Simple Match Data (GNU Emacs Lisp Reference Manual)]] for extracting group data from a found match. |
...
Some translations between Python ~os~ and ~os.path~ methods to Elisp.

#+begin_example
p: string describing a path to a file or directory
f: string describing a path to a filename
d: string describing a path to a directory
e: string describing a file extension
#+end_example


| Python                      | Elisp                                                                                                     | Notes                                                                           |
|-----------------------------+-----------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------|
| ~os.listdir(d)~               | ~(directory-files d)~                                                                                       |                                                                                 |
| ~os.stat(p)~                  | ~(file-attributes p)~                                                                                       | ~stat~ and ~file-attributes~ are not isomorphic.                                    |
| ~os.getcwd()~                 | ~default-directory~                                                                                         | ~getcwd~ is a function, ~default-directory~ is a variable.                          |
|                             | ~(directory-files-and-attributes d)~                                                                        | Combines ~directory-files~ and ~file-attributes~.                                   |
| ~os.path.abspath(p)~          | ~(expand-file-name p)~                                                                                      |                                                                                 |
| ~os.path.exists(p)~           | ~(file-exists-p p)~                                                                                         |                                                                                 |
| ~os.path.isfile(p)~           | ~(file-regular-p p)~                                                                                        |                                                                                 |
| ~os.path.isdir(p)~            | ~(file-directory-p p)~                                                                                      |                                                                                 |
| ~os.path.islink(p)~           | ~(file-symlink-p p)~                                                                                        |                                                                                 |
| ~os.path.join(d, p)~          | ~(file-name-concat d p)~                                                                                    |                                                                                 |
| ~os.mkdir(p)~, ~os.makedirs(p)~ | ~(make-directory p)~                                                                                        |                                                                                 |
| ~os.path.basename(p)~         | ~(car (last (file-name-split p)))~                                                                          |                                                                                 |
| ~os.path.splitext(f)~         | ~(list (file-name-concat (file-name-directory f) (file-name-base f)) (concat "." (file-name-extension f)))~ | ~splitext~ will include full path, ~file-name-base~ will use ~file-name-nondirectory~ |
| ~os.path.split(f)[1]~         | ~(file-name-nondirectory f)~                                                                                |                                                                                 |
| ~os.path.split(f)[0]~         | ~(file-name-directory f)~                                                                                   |                                                                                 |
|                             | ~(file-name-with-extension f e)~                                                                            |                                                                                 |

Sorting a directory list by modification date, newest first.

#+BEGIN_SRC elisp :lexical no
  (sort (directory-files-and-attributes d)
...
The parsing of URLs in Python is handled by the ~urllib.parse~ module ([[https://docs.python.org/3/library/urllib.parse.html][urllib.parse]]). The Elisp analog is the URL library ([[info:url#Top]]). These libraries are idiomatic in their handling of URLs, so close reading is highly recommended. That said they both have the same intent: to make working with URLs easier. Listed below are some Python methods with their Elisp function analogs.

#+begin_example
u: url as string
urlobj: url object
s: string
q: collection of query arguments
qs: query string
#+end_example

| Python (~urllib.parse~)       | Elisp                                            | Notes                              |
|-----------------------------+--------------------------------------------------+------------------------------------|
| ~urlsplit(u)~                 | ~(url-generic-parse-url u)~                        | This functions are /not/ isomorphic. |
| ~urlunsplit(urlobj)~          | ~(url-recreate-url urlobj)~                        | This functions are /not/ isomorphic. |
| ~quote(s)~                    | ~(url-hexify-string s)~                            |                                    |
| ~unquote(s)~                  | ~(url-unhex-string s)~                             |                                    |
| ~quote_plus(s)~               | ~(string-replace "%20" "+" (url-hexify-string s))~ |                                    |
| ~unquote_plus(s)~             | ~(url-unhex-string (string-replace "+" "%20" s))~  |                                    |
| ~urlencode(q)~                | ~(url-build-query-string q)~                       | This functions are /not/ isomorphic. |
| ~parse_qs(qs)~, ~parse_qsl(qs)~ | ~(url-parse-query-string qs)~                      | This functions are /not/ isomorphic. |

* License
[[https://mirrors.creativecommons.org/presskit/buttons/88x31/svg/by.svg]]\\
Copyright 2025-2026 Charles Y. Choi. This work is openly licensed via [[https://creativecommons.org/licenses/by/4.0/][CC BY 4.0]]. 
